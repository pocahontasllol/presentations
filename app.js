...
slides.forEach((slide,idx) =>  {
    ...
    const options = { defaults: {deration:1,ease:'power2.inOut'}}
    const timeLine = gsap.timeline(options)


    timeLine.fromTo(revealImg,{x:'0%'},{x:'100%'})
    timeLine.fromTo(img, {scale:3},{scale:1})

    timeLine.fromTo(revealText,{x:'0%'},{x:'-100%'})
    ...
    const pageOptions = {defoults :{deration:1,ease:'power1.inOut'} }
